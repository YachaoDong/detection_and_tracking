...
from utils.general import (DATASETS_DIR, LOGGER, NUM_THREADS, check_dataset, check_requirements, check_yaml, clean_str,
                           cv2, xyn2xy, xywh2xyxy, xywhn2xyxy, xyxy2xywhn, is_colab, is_kaggle)
...
def img2label_paths(img_paths):
    ...
    sa, sb = f'{os.sep}images{os.sep}', f'{os.sep}images{os.sep}'  # /images/, /labels/ substrings
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        try:
            ...
            cache, exists = self.cache_labels(cache_path, prefix, datahyp), False  # cache
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix='', dataYaml = ""):
        # Cache dataset labels, check images and read shapes
        x = {}  # dict
        nm, nf, ne, nc, msgs = 0, 0, 0, 0, []  # number missing, found, empty, corrupt, messages
        desc = f"{prefix}Scanning '{path.parent / path.stem}' images and labels..."
        with Pool(NUM_THREADS) as pool:
            pbar = tqdm(pool.imap(verify_image_label, zip(self.im_files, self.label_files, repeat(prefix), repeat(dataYaml))),
                        desc=desc,
...
def verify_image_label(args):
    ...
    im_file, lb_file, prefix, dataHyp = args
    with open(dataHyp) as f:
        nameList = yaml.safe_load(f)["names"]
    nameDict = {}
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            for obj in objects:
                ...
                lb.append(np.array([nameDict[name], cx/width, cy/height, w/width, h/height], dtype=np.float32))
            lb = np.array(lb)
            nl = len(lb)
            if nl:
            ...
            nm = 1  # label missing
            msg = f'{prefix}WARNING: {lb_file}: not find'
            print(msg)
            lb = np.zeros((0, 5), dtype=np.float32)
