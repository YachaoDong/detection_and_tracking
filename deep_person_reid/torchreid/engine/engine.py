...
class Engine(object):
    ...
    def save_model(self, epoch, rank1, save_dir, is_best=False):
        ...
        print(f"save_model names:{names}")

        for name in names:
            print(f"osp.join(save_dir, name):{osp.join(save_dir, name)}")
            save_checkpoint(
                {
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            if (self.epoch + 1) >= start_eval \
               ...
               and (self.epoch + 1) != self.max_epoch:
                ...
                # 保存 latest 的model
                print(f"saving {self.epoch} epoch model.....")
                self.save_model(self.epoch, rank1, save_dir, is_best=False)
                if rank1 > self.best_rank1:
                    print(f"saving best_rank1:{rank1} epoch model.....")
                    self.save_model(self.epoch, rank1, save_dir, is_best=True)
